...
from podstatus import PodStatusArchive,settings
...
def run():
    try:
        ...
        if args.max_archive_times:
            PodStatusArchive.get_instance(settings).archive(args.max_archive_times)
        else:
            PodStatusArchive.get_instance(settings).archive()
    except:
